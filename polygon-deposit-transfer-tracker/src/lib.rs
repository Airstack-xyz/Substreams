...
fn map_transfers(blk: eth::Block) -> Result<Transfers, Error> {
    ...
    for log in blk.logs() {
        ...
        let tx_from =
            helpers::utils::format_with_0x(Hex(log.receipt.transaction.from.clone()).to_string());
        let tx_to =
            helpers::utils::format_with_0x(Hex(log.receipt.transaction.to.clone()).to_string());


        // deposit transactions have zero from & to address 
        // https://polygonscan.com/tx/0xe5051426f12327b564269bec87ce8ebec26e605373fd07621ec7a152cc08e223
        if tx_from != ZERO_ADDRESS || tx_to != ZERO_ADDRESS {
            continue;
        }
